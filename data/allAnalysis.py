...
def writeCSV(filename, titles, delay, flights):
    ...
    for i, row in enumerate(delay):
        writer.write(', '.join(row[0]) + ',' + str(round(row[-1],1))+ ',' + str(flights[i][-1])  + '\n')
    writer.close()
...
if __name__ == "__main__":
    ...
    for d in data:
        ...
        delay[(d[0],d[1],d[3],d[4])] = delay.get((d[0],d[1],d[3],d[4]), 0) + int(d[2]) if d[2] != 'NA' else 0

    import operator
    writeCSV('../data-all/' + year + '-allIn.csv', titles[:2]+titles[3:] + ['Delay', 'Flgihts'], 
